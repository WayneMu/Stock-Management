...
app.post('/submitbill', checkAuthenticated,(req, res) => {
  ...
  var   ExpirationDate = date_format.getDate()+ '/' +(parseInt(date_format.getMonth()+1)).toString() + '/'+ date_format.getFullYear()
  ...
  console.log(`\nINSERT Array = `)
  console.log(data)
  let sql = `INSERT INTO ordersdb(ItemID,ItemName,Category,Brand,Size,Amount,ExpirationDate,CustomerNumber,TransactionDate,TransactionTime,TransactionID,TDay,TMonth,TYear) VALUES ? `
...
app.post('/submitstock', checkAuthenticated,(req, res) => {
  ...
  let sql = `INSERT INTO stockdb(ItemID,ItemName,Category,Brand,Size,Amount,ExpirationDate,StockDate,StockTime,TDay,TMonth,TYear) VALUES ? `
  let query = mysqlConnection.query(sql,[ data], (err, rows, fields)=>{
    if(!err)
    {
